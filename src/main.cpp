...
#if defined(NDEBUG)
# error "Advertcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7a42e5ec1bc9d1c80bcc0bf40d8c05b9dbf28ae7f25ceba1c7d41aa5e4a8a9d7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Advertcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Advertcoin Signed Message:\n";
...
{
    // Advertcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 200 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 52000000 * COIN;
            }

    // Subsidy is cut in half every 520000 blocks
    nSubsidy >>= (nHeight / 520000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Advertcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Advertcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Advertcoin na Lua";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041dfdfda3df94ecb9d072859d15dcc0e3f60bebb6537a3ea11152a12ed8227599060c6c408583567bc24f15780bcb6f42de1848b19d739b808d7dd200f5a12188") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526000447;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2927036;
        ...
        assert(block.hashMerkleRoot == uint256("0xebb5d3da8d173dd52d09c7821c85017b20c488caa0a0a287141027085def0bda"));
...
unsigned char pchMessageStart[4] = { 0xd3, 0xae, 0xc7, 0x06 }; // Advertcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AdvertcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdvertcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdvertcoinMiner(CWallet *pwallet)
{
    printf("AdvertcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("advertcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AdvertcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AdvertcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdvertcoinMiner, pwallet));
}
